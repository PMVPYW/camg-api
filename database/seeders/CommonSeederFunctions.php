...
class CommonSeederFunctions extends Seeder
{
    ...
    public static function storePhotoEntidade($name)
    {
        $n_array = explode('.', $name);
        $file_type = $n_array[count($n_array) - 1];
        $file_name_to_store = substr(base64_encode(microtime()), 3, 6) . '.' . $file_type;
        File::copy("database/seeders/fotos/entidades/" . $name, storage_path('app/public/entidades/' . $file_name_to_store));
        return $file_name_to_store;
    }

    public static function storePhotoDeclaracoes($name)
    {
        $n_array = explode('.', $name);
        $file_type = $n_array[count($n_array) - 1];
        $file_name_to_store = substr(base64_encode(microtime()), 3, 6) . '.' . $file_type;
        File::copy("database/seeders/fotos/declaracoes/" . $name, storage_path('app/public/declaracoes/' . $file_name_to_store));
        return $file_name_to_store;
    }
}
